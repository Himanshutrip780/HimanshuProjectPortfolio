...
export class RealtimeService {
  ...
  async connect(): Promise<void> {
    ...
    this.connection = new HubConnectionBuilder()
      .withUrl(apiUrl(environment.apiGatewayUrl, '/hubs/workflow.io'), {
  ...
  async joinProject(projectId: string): Promise<void> {
    await this.connect();
    const orgId = this.tokenStorage.getActiveOrganizationId();
    await this.connection?.invoke('JoinProject', projectId, orgId);
  }

  async joinTask(taskId: string): Promise<void> {
    await this.connect();
    const orgId = this.tokenStorage.getActiveOrganizationId();
    await this.connection?.invoke('JoinTask', taskId, orgId);
  }
